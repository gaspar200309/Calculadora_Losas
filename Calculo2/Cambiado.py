...
class Principal:
    ...
    def setup_ec3(self):
        ...
        labels = [
            ...
            ('Vu', 0.03, 0.256), ('b', 0.230, 0.256), ('h', 0.455, 0.256),
        ]
    ...
    def calcular_d(self):
        try:
            ...
            rec = 0.025  # Valor estático
            Asøprincipal = 0.010  # Valor estático
            d = h - rec - (Asøprincipal / 2)
            self.entryD.delete(0, tk.END)
            self.entryD.insert(0, f'{d:.4f}')
            messagebox.showinfo("Resultado", f"d = {d:.4f}")
        except ValueError:
            messagebox.showerror("Error", "Por favor, ingrese un valor numérico válido para h.")
